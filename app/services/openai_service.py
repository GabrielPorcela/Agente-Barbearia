...
def _get_client() -> OpenAI:
    ...
    if _client is None:
        ...
        # timeout/max_retries limitam o tempo total bem abaixo do timeout
        # padrão de worker do gunicorn (30s). Sem isso, uma chamada lenta
        # à OpenAI (o SDK usa 600s de timeout por padrão) pode fazer o
        # gunicorn matar o worker no meio da requisição -- antes que o
        # try/except deste módulo tenha chance de capturar o erro e
        # devolver o fallback seguro ("conversa_normal").
        _client = OpenAI(api_key=api_key, timeout=10.0, max_retries=1)
    return _client
